...
The today's lecture will be about the fundamental problem of finding a cut of least conductance in a graph, called \textsc{Sparsest Cut} in the literature.
...
\begin{notation}
    We define $\lVert E(S,\overline{S})\rVert$ as the number of edges that cross the cut  $(S,\overline{S})$, i.e., have one end point in $S$ and the other in $\overline{S}$.
\end{notation}    
...
This definition of $\Phi(S)$ is a \emph{dimension-less} quantity and lies always between $0$ and $1$.

Looking for the conductance of a graph has also crucial real-world applications. Indeed, it plays a central role in the design of recursive algorithms, image segmentation, community detection, and clustering.
...
NDLR: We can notice this problem is obviously \NP, thus, it is \NP-complete. Indeed the decision problem associated is to find a cut with conductance $\leqslant K$, for any $K$. Such a cut is a polynomial certificate.
...
We just proved that the h-value lead us to capture the conductance of a graph up to a factor 2. But the problem remains \NP-hard.
...
$\mu$ is the uniform probability measure defined on the set of vertices $V$ induced by the degrees.
