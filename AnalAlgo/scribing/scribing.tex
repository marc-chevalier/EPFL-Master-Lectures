...
Indeed, $\lvert y_i -y_j \rvert \neq (y_i-y_j)^2$. We previously used square-Euclidian distances for convenience in computations. But, it makes more sense to consider $\left|y_i-y_j\right|$.
...
To prove the other inequality, let's take $y_1 < \cdots < y_n$ be an embedding of $G$ which minimises $\frac{\esp{\lvert y_i-y_j\rvert}}{2\esp{\lvert y_i \rvert}}$. We assume that the $y_i$ are pairwise distinct and that $\mu_{\frac{1}{2}}(y) = 0$.

in order to compute the term $\left|y_j-y_i\right|$, we will use the simple decomposition of substractions in the first Euclidian norm :
