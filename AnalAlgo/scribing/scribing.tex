...
This expression is close to a mathematical program finding an eigenvalue at the exception of the $D$ in the denominator. To reduce this problem to finding the second smallest eigenvalue, we can apply the following change of variable: $y = D^{\frac{1}{2}} x$ where 
\[
...
Indeed, $\lvert y_i -y_j \rvert \neq (y_i-y_j)^2$. We previously used square-Euclidean distances for convenience in computations. But, it makes more sense to consider $\left|y_i-y_j\right|$.
...
\paragraph{Part 2}
\[
    \frac{\esp[ij\la\nu]{(x_i-x_j)^2}}{\esp[(i,j)\la\mu\times\mu]{(x_i-x_j)^2}} \rightsquigarrow \overbrace{x^*}^{rounding\,\, 2\sqrt{\lambda_2(\L)}}
\]
...
Let's take $y_1 < \cdots < y_n$ be an embedding of $G$ which minimises $\frac{\esp{\lvert y_i-y_j\rvert}}{2\esp{\lvert y_i \rvert}}$. We assume that the $y_i$ are pairwise distinct and that $\mu_{\frac{1}{2}}(y) = 0$.

In order to compute the term $\left|y_j-y_i\right|$, we will use the simple decomposition of subtractions in the first Euclidean norm :

\[
    y_j-y_i=(y_j-y_{j-1})+(y_{j-1}-y_{j-2})+\dots+(y_{i+1}-y_i)
\]
    ...
    Hence, $\left(y_{i+1}-y_i\right) = \left|E\left(S_i,\,\overline{S_i}\right)\right|$.
...
\[
    \sum\limits_{i=1}^n d_i\left|y_i\right| = \sum\limits_{i=1}^{n-1} (y_{i+1} - y_i) \min(\underbrace{\vol S_i, \vol \overline{S_i}}_{d_i})
\]
...
because $\frac{a_1+a_2}{b_1+b_2} \geqslant \min\set{\frac{a_1}{b_1},\frac{a_2}{b_2}}$.
...
So, it remains 
\[
    \sum\limits_{i=1}^n d_i\left|y_i\right| = \sum\limits_{i=1}^{n-1} (y_{i+1} - y_i) \min(\vol S_i, \vol \overline{S_i})
\]
to prove.
...
Actually, this proof generalizes easily. We just have to set $y_k = \max\limits_{i \in \N} \{y_i,\, y_i<0\}$.
